...
/*
 ...
 Steep Parallax Mapping 陡峭视差映射 会出现 表面分裂成层(特别更低的视角), 因为分层可能会错失相交点; 可以增加层级数目，但性能会下降
 另外 MSAA 不能解决，因为它只适用于几何体的边缘，而不适用于纹理效果。
 我们不能写入每个片段的深度缓冲区吗？
 这在足够先进的硬件上确实是可能的，从而可以正确地将其他几何体与高度场相交并应用阴影。 不过，它并不便宜。
 Parallax Occlusion Mapping 视差遮蔽映射, 假设表面在样本点之间是线性的，这可以防止最明显的分层伪影。
 但是，它不能帮助我们检测我们何时错过了步骤之间的交叉点。 我们仍然需要很多样本来处理"小特征、轮廓和浅角度"。
 
 Relief Parallax Mapping 浮雕视差贴图  二分查找 每走一步，距离减半，直到到达目的地
 参考实现 https://catlikecoding.com/unity/tutorials/rendering/part-20/
