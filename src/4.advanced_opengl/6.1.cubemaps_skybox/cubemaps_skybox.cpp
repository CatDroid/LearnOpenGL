...
int main()
{
    ...
    float skyboxVertices[] = {  // 天空盒子 只有-1到1 这么大
        // positions          
        -1.0f,  1.0f, -1.0f,
        -1.0f, -1.0f, -1.0f,
         1.0f, -1.0f, -1.0f,
	...
	*/
    ...
    while (!glfwWindowShouldClose(window))
    {
    ...
    
        {
            ...
            glDepthMask(GL_FALSE);      // 不做深度写入 但是还是保留深度测试
            skyboxShader.use();
...

        {
            ...
            // 因为shader中 gl_Postion=xyww  z透视除法后横等于1 如果只用LESS的话，深度测试会不通过
            // 这里深度写入还是打开的，但是天空盒的深度永远是1，所以不会影响其他物体
            // 相当于如果要画一个物体永远在其他物体的最后面，只要shader中使用xyww即可(xyzw是透视除法后的)，不用做一个2000x2000x2000很大的盒子
            ...
            glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
            skyboxShader.setMat4("view", view);
            skyboxShader.setMat4("projection", projection); // skybox 为什么还要投影?? 直接把-1到1传过去不就好了??
